...
│   └── subscriptions      # Gestão de Planos e Assinaturas via Stripe
...
Gestão de planos (Free, Pro, Enterprise) e pagamentos integrados ao Stripe.
| Operação | Descrição |
|----------|-----------|
| Planos & Checkout | Listagem de planos dinâmicos vindos do Stripe e criação de seções de Checkout. |
| Webhooks | Sincronização em tempo-real de faturas, pagamentos e cancelamentos via Stripe Webhooks. |
| Portal do Cliente | Geração de links seguros para o Stripe Customer Portal (gerenciamento de assinaturas). |
...
- `user_profiles` ➔ Configurações individuais e dados demográficos do usuário estendidos.
- `subscriptions` ➔ Armazena o vínculo ativo com o Stripe (customer_id, status do plano, renovação).
- `usage_limits` ➔ Engine de cotas restritivo de rate-limiters e Features (AI Credits, Exports) amarrado ao tier de assinatura.
...
| `STRIPE_SECRET_KEY` / `WEBHOOK_SECRET` | ✅ | Chaves da API do Stripe para processamento de assinaturas e eventos webhook. |
| `STRIPE_PRO_PRICE_ID` / `STRIPE_ENTERPRISE_...` | ✅ | Identificadores dos preços e produtos configurados no Dashboard do Stripe. |
